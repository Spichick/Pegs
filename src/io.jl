...
"""
function readInputFile(inputFile::String) # 输出Matrix
    file = open(inputFile)
    lines = readlines(file)
    close(file)

    line = size(lines, 1) 
    count_comma = count(==(','), lines[1])
    row = 1 +  count_comma # 数第一行的逗号数+1
    init_ = Matrix{Int64}(fill(-1, line, row))
    # println("line:", line)
    # println("row:", row)
# TODO:
    ...
    for i in 1:line
        ...
        # println()
        flag = 0
        for j in 1:row + count_comma
            if lines[i][j]==' ' || lines[i][j]=='0' || lines[i][j]=='1'
                flag += 1
                # println("|",flag)
            end
            if lines[i][j]!='\n' && lines[i][j]!=',' && lines[i][j] != ' ' # 0/1
                number = parse(Int, lines[i][j]) # 将第i行第j个字符转为数字
                init_[i, flag] = number
                # println(number,"|",flag)
            end
